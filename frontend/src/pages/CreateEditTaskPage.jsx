...
export default function CreateEditTaskPage() {
  ...
  useEffect(() => {
    ...
    api.get('/admin/users').then(r => setUsers(r.data.filter(u => u.role === 'executor'))).catch(() => {});
    if (isEdit) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) { await api.put(`/tasks/${id}`, payload); toast.success('Задача обновлена'); }
      else        { const r = await api.post('/tasks', payload); toast.success('Задача создана'); navigate(`/tasks/${r.data.id}`); return; }
      navigate(`/tasks/${id}`);
